...
def process_number(message, edit_msg=None, service_name="Unknown", rid=None):
    ...
    # ✅ ২টা নাম্বার নাও (নতুন Nexus API)
    nums      = []
    countries = []
    max_retries = 5
    ...
    for attempt in range(max_retries):
        try:
            # নতুন API: GET /api/v1/numbers with params
            params = {
                "range": "bd",  # Bangladesh
                "sid": "wa",    # WhatsApp
                "no_plus": False,
            ...
            if data.get("ok") and data.get("number"):
                ...
                country  = "Bangladesh"  # নতুন API এ country পাওয়া যায় না, তবে range থেকে বুঝা যায়
                
                if full_num not in nums:
                    nums.append(full_num)
                    countries.append(country)
                    user_ranges[chat_id] = data.get("id")  # API ID সংরক্ষণ করুন
                break
    ...
    for attempt in range(3):
        try:
            ...
            if data.get("ok") and data.get("number"):
                full_num2 = str(data["number"]).replace("+", "")
                country2  = "Bangladesh"
